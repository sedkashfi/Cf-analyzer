...
st.markdown("**تحلیل Acceptedها بر اساس ریتینگ + سن سوال**")
    ...
    with st.spinner(f"در حال دریافت اطلاعات @{handle} ..."):
        try:
            ...
            if not details:
                ...
                st.success(f"@{handle} — {len(df)} Accepted در ۷ روز")
                ...
                with col3:
                    st.metric("جدید", sum(1 for d in details if d['Category'] == "جدید (۲۰۲۴+)"))
...
st.caption("Made with ❤️ | بروزرسانی دستی")
